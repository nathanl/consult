defmodule Consult.ConversationController do
  ...
  def set_tags(conn, %{"tag_ids" => tag_ids, "id" => conversation_id}) do
    ...
    {:ok, to_delete, to_insert} =
      ConversationTag.updates_for(
        existing_taggings,
        conversation_id,
        checked_values_from(tag_ids)
      )

    to_insert =
      to_insert
      |> Enum.map(fn ct ->
        ConversationTag.changeset(%ConversationTag{}, %{
          tag_id: ct.tag_id,
    ...
    multi =
      to_delete
      |> Enum.reduce(Ecto.Multi.new(), fn record, multi_acc ->
