defmodule Consult.ConversationChannel do
    ...
    conversation =
      Consult.repo().one!(
        Ecto.Query.from(
          ...
          preload: [messages: ^Message.reverse_sequential(Message)]
        )
      )

    conversation.messages |> :lists.reverse()
  ...
  def handle_in("new_msg", %{"body" => body}, socket) do
    %Conversation{ended_at: nil} =
      Consult.repo().get_by!(Conversation, id: socket.assigns[:conversation_id])
  ...
  defp message_for_channel(message) do
    %{
      ...
      user_public_identifier:
        Consult.Token.user_identifier(%{id: message.sender_id, name: message.sender_name}),
